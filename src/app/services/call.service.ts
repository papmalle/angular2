...
export class CallService {
  ...
  */
  ...
  getListAppareils(): Observable<Call[]> {
    if (this.calls) {
      return of(this.calls);
    }

    return this.httpClient.get<Call[]>('http://localhost:8085/api/calls').pipe(
      // throttleTime(5000)
      map(((value, index) => this.calls = value))
    );
  }

/*  emitAppareilSubject() {
    this.callsSubject.next(this.calls.slice());
  }*/
}
